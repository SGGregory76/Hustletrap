// mission-engine.js
// Fully self-contained mission engine with updated list descriptions and flex button layout

// 1. Log engine load
console.log("✅ MissionEngine loaded with updated styles");

// 2. Inject scoped CSS for mission panel, list styling, icons, and flex buttons
(function() {
  const css = `
    /* Panel container */
    #mission-container {
    ...
    /* Force white text & remove link decorations */
...
// 4. Missions definitions with HTML lists
const Missions = {
  "find-burner-os": {
    title: "Something in the Alley",
    description: `
      <ul>
        <li>Wake up cold, alone, and hungry on a backstreet bench</li>
        <li>Head’s pounding and pockets empty</li>
        <li>Spot a cracked smartphone half-buried in rain-soaked trash</li>
      </ul>
      What do you do?`,
    options: [
  ...
  "delivery-route-test": {
    ...
    description: `
      <ul>
        <li>Route A: docks (slow, low heat)</li>
        <li>Route B: badlands (fast, medium heat)</li>
        <li>Route C: interstate (fastest, high heat)</li>
      </ul>
...
const MissionEngine = {
  ...
  start(id) {
    ...
    if (!mission) return console.error(`No mission: ${id}`);
    this.current = mission;
    this.render();
    if (mission.timerSeconds) this.runTimer(mission.timerSeconds);
  },
  render() {
