...
function initNPC(npcId, defaultInventory, allowResourceSales = false) {
  ...
  // Log what the player gave the NPC
...
function renderNPCInventory(npcId) {
  console.log("Rendering inventory for:", npcId);
  const container = document.getElementById("npcInventory");
  if (!container) {
    console.warn("Missing #npcInventory element on page.");
    return;
  }
  let npcInventory = JSON.parse(localStorage.getItem(npcId + "_inventory") || "[]");

  // If inventory is blank or invalid, force reset it for debug
  if (!Array.isArray(npcInventory) || npcInventory.length !== 5) {
    console.warn("Invalid or missing inventory. Forcing test inventory.");
    npcInventory = [
      { name: "LSD", icon: "🔮", qty: 1, price: 60, type: "drug" },
      { name: "Glass Flask", icon: "⚗️", qty: 1, price: 20, type: "resource" },
      null, null, null
    ];
    localStorage.setItem(npcId + "_inventory", JSON.stringify(npcInventory));
  }
  ...
  npcInventory.forEach(item => {
    if (item) {
      ...
      const canBuy = !item.consumed;
      container.innerHTML += `
        <div class="inventory-slot">
          <span>${item.icon} <strong>${item.name}</strong> x${item.qty} - $${item.price}</span>
          ${canBuy ? `<button onclick="buyFromNPC('${npcId}', ${i})">Buy</button>` : ''}
        </div>
      `;
